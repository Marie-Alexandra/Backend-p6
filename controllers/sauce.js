...
exports.createSauce = (req, res, next) => {
  ...
  sauce.save()
    ...
    .catch(error => res.status(400).json({ error: 'createsauce ' + error }));
};
...
exports.modifySauce = (req, res, next) => {
  ...
  Sauce.updateOne({ _id: req.params.id }, { ...sauceObject, _id: req.params.id })
    ...
    .catch(error => res.status(400).json({ error: 'modifysauce ' + error }));
};
    ...
    .then(sauce => { //Quand on le trouve
      ...
      fs.unlink(`images/${filename}`, () => { //On supprime ce fichier (ici l'image)
        Sauce.deleteOne({ _id: req.params.id }) //Puis on supprime l'objet de la base de données
          ...
          .catch(error => res.status(400).json({ error: 'delete sauce ' + error  }));
      });
    })
    .catch(error => res.status(500).json({ error: 'find one delete ' + error }));
};
...
exports.likeASauce = function (req, res, next) {
  Sauce.findOne({ _id: req.params.id })
          ...
          if (!likedSauce.usersLiked.includes(req.body.userId) && req.body.like === 1) {
            Sauce.updateOne({ _id: req.params.id },
              ...
              .catch(function (error) {
                res.status(400).json({ error: 'user like sauce ' + error });
              });
          }
          break;
        ...
        case -1:
          if (!likedSauce.usersDisliked.includes(req.body.userId) && req.body.like === -1) {
            Sauce.updateOne({ _id: req.params.id },
              ...
              .catch(function (error) {
                res.status(400).json({ error: 'user dislike sauce ' + error });
              });
          }
          break;
        ...
        case 0:
          if (likedSauce.usersLiked.includes(req.body.userId)) {
            Sauce.updateOne({ _id: req.params.id },
              ...
              .catch(function (error) {
                res.status(400).json({ error: 'annulation du like ' + error });
              });
          }
          ...
          if (likedSauce.usersDisliked.includes(req.body.userId)) {
            Sauce.updateOne(
              ...
              .catch(function (error) {
                res.status(400).json({ error: 'annulation du dislake ' + error });
              });
          }
          break;
      }
    ...
    .catch(function (error) {
      res.status(404).json({ error: 'find one like sauce ' + error });
    });
};