  ...
  bcrypt.hash(req.body.password, 10) 
    .then(hash => {
      ...
      user.save() //Pour sauvegarder l'utilisateur dans la base de données
        ...
        .catch(error => res.status(400).json({ error: 'signup ' + error }));
    })
    .catch(error => res.status(500).json({ error: 'bcrypt h ' + error }));
};
  ...
  User.findOne({ email: req.body.email })
    .then(user => {
      ...
      bcrypt.compare(req.body.password, user.password)
        ...
        .catch(error => res.status(500).json({ error: ' bcrypt c ' + error }));
    })
    // Erreur serveur
